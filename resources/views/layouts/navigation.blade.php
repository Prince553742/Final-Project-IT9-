<aside class="fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-100 flex flex-col justify-between shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-50 transition-all duration-300">
    
    <div>
        <div class="h-20 flex items-center px-6 border-b border-gray-50/80">
            <a href="#" class="flex items-center gap-3 group">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center text-white font-black text-xl shadow-md group-hover:shadow-lg group-hover:scale-105 transition-all">
                    M
                </div>
        ...
        <nav class="p-4 space-y-1 mt-2">
            ...
            @php
                // Standardize the role string (adjust this based on your actual user model column)
    ...
    <div class="p-4 border-t border-gray-100 bg-gray-50/50">
        <div class="flex items-center justify-between p-2 rounded-xl hover:bg-white hover:shadow-sm transition-all cursor-pointer group">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-indigo-100 text-indigo-700 font-black text-sm flex items-center justify-center border border-indigo-200">
                    {{ strtoupper(substr(auth()->user()->name ?? 'U', 0, 1)) }}
                </div>
                <div class="flex flex-col">
                    <span class="text-sm font-extrabold text-gray-900 leading-tight">{{ auth()->user()->name ?? 'User' }}</span>
                    <span class="text-xs font-semibold text-gray-500 capitalize">{{ $userRole }}</span>
                </div>
            </div>
